...
class DBHandler:
    ...
    def update_root_path(self, root_path, target_directory, config_id):
        self.cursor.execute('UPDATE config SET rootpath = ?, target_directory = ? WHERE config_id = ?', (root_path, target_directory, config_id))
        self.conn.commit()
