...
def new_webdav_directory(url, token, path, logger):
    refresh_url = f"{url}/api/fs/mkdir"  # 动态构建 API 刷新路径
    headers = {
        "Authorization": f"{token}"
    }
    path = path.replace('/dav', '')
    logger.info(f"new_webdav_directory to be created: {path}")
    payload = {
        "path": path
    }
    ...
    try:
        response = requests.post(refresh_url, headers=headers, json=payload)
        if response.status_code == 200:
            logger.info(f"创建目录 '{path}' 成功。")
        else:
            logger.error(f"创建目录 '{path}' 时出错: {response.status_code}, {response.text}")
    except Exception as e:
        logger.error(f"创建目录 '{path}' 时发生异常: {e}")

def process_with_cache(webdav, config, script_config, config_id, size_threshold, db_handler, logger):
    ...
    if protocol and host and port:
        ...
        if username and password:
            ...
            if token:
                directory_info = refresh_webdav_directory(url, token, root_directory, logger)
            else:
    ...
    if config.get('update_mode') == 'incremental':
        ...
        if new_directory == 1:
            ...
            if directory_info is not None and directory_info['total'] >= max_file_count:
                ...
                new_webdav_directory(url, token, new_root_directory, logger)
                ...
                new_target_directory = target_directory.replace(target_directory.split('/')[-1], new_directory_name)
                logger.info(f"new_target_directory {new_target_directory}")
                db_handler.update_root_path(new_root_directory, new_target_directory, config_id)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 输出配置信息到日志
        logger.info(
            f"正在使用配置ID: {config_id} 运行，目标地址: {config['protocol']}://{config['host']}:{config['port']}, 配置名: {config['config_name']}"
        )
        ...
        try:
            ...
            process_with_cache(webdav, config, script_config, config_id, script_config['size_threshold'], db_handler, logger)
        except Exception as e:
